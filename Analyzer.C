...
void Analyzer::CutFlowProcessor(TString url,  string myKey, TString cutflowType, TFile *outFile_){
  ...
  for(int i=0; i<nEntries; ++i){
    ...
    if(vZ.mass() > 60 && j_final.size()!=0 ) isControlSel = true;
    //---------------------------------------------------//
    ...
    for(size_t ijet = 0; ijet < j_final.size(); ijet++){
      ...
      if(jetPt > 100 && jetEta < 2.5 && dR1 > 0.8 && dR2 > 0.8 && vZ.Pt() > 100) isPreSel = true;
    ...
    for(size_t ijet = 0; ijet < j_final.size(); ijet++){
      ...
      double jetPt = jetPtWithJESJER(pfJets[ind_jet], jes, jer);
      if(isControlSel && isBTagVeto && isPreSel && 
		      vZ.M()> 200 && jetPt >200 && ak8Pmass_ > 70 && ak8Pmass_ < 110 && ak8Tau21 < 0.60){
        isZTagSel = true; 
	allZjet.push_back(ijet);
      }
    }
    if(isControlSel && isBTagVeto && isPreSel && isZTagSel){
      ...
      fillHisto2D(outFile_, cutflowType_,"ZTag", "mlZmin_mlZmax",500, 0, 10000, mlZmin, 500, 0, 10000, mlZmax, evtWeight);
      //fill histos for jets
      for(size_t ijet = 0; ijet < j_final.size(); ijet++){
        int ind_jet = j_final[ijet];
        double jetPt = jetPtWithJESJER(pfJets[ind_jet], jes, jer);
...
void Analyzer::processEvents(){ 
  ...
  //CutFlowAnalysis("root://se01.indiacms.res.in:1094//cms/store/user/sthakur/ntuple_for2016Data_MuData_20190410/MuData_20190410/sMuRunEv1_MuData_20190410/SingleMuon/sMuRunEv1_MuData_20190410/190410_163727/0000/sMuRunEv1_MuData_20190410_Ntuple_62.root");
   //====================================
  //condor submission
