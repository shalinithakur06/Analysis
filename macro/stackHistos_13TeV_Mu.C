...
void example_stack(TString dir, TString histname, TString xaxis_title, int bin, bool log=false, bool drawdata=true, bool ratio=false, bool drawsignal=false, bool axisrange=false, double xmin=0, double xmax=10, bool label=false, double unc=false){
  ...
  //Legends
  TLegend* leg = new TLegend(0.8218792,0.6061504,0.9212081,0.8898861,NULL,"brNDC");
  ...
  */
  ...
  else stackHisto(fQCD, "QCD", dir, histname, 3, 1.20, axisrange, xmin, xmax, MuptStack, hMC, leg);
  stackHisto(fWJ, "W+ jets", dir, histname, 6 , 1.03487, axisrange, xmin, xmax, MuptStack, hMC, leg);
  stackHisto(fTT,"t#bar{t}", dir, histname, 433, 1, axisrange, xmin, xmax, MuptStack, hMC, leg);
  stackHisto(fDY, "Z+jets", dir, histname, 9, 1.02788, axisrange, xmin, xmax, MuptStack, hMC, leg);
  ...
  MuptStack->GetYaxis()->SetTitleOffset(1.00);
  ...
  leg->AddEntry(hSig, "500* Signal","L"); 
  leg->Draw();
  ...
  //-------------------------------------///
  //signal
  TPaveText *cct = paveText(0.8218792,0.5561504,0.9212081,0.6098861, 0, 19, 1, 0, 132);
  cct->SetTextSize(0.039);
  cct->AddText("M_{l^{*}} = 2 TeV");
  ...
  if(ratio){
    ...
    hRatio->GetYaxis()->SetTitleSize(0.20); hRatio->GetXaxis()->SetTitleSize(0.20);
  ...
  if(label){
    TString steps[8]={"muon trig","= 2 muon","muon SF","RelIso < 0.15", "M_{#mu1#mu2} #geq 60","PreSel", "Z-Tag",""}; 
...
void example_stack_ZTag(TString dir="ZTag"){
 example_stack(dir,"pt_1stMu","Pt^{#mu1}[GeV]", 1, true,true,true,true, true,   0.0,    500.0, false, false);
 example_stack(dir,"pt_2ndMu","Pt^{#mu2}[GeV]", 1, true,true,true,true, true,   0.0,    500.0, false, false);
 example_stack(dir,"mjj","M_{#mu1#mu2}",1,true,true,true,true, true,               50,    2000.0, false, false);
 example_stack(dir,"ak8Pmass","Mass_{jets}^{Pruned} [GeV]", 1, true,true,true,true, true, 100,    2000,  false, false);
 example_stack(dir,"ak8Tau21","#tau21_{jets}", 1, true,true,true,true, true, 0,    1.5,  false, false);
 example_stack(dir,"pt_jet","Pt^{jets}[GeV]", 1, true,true,true,true, true, 100,    2000,  false, false);
 example_stack(dir,"eta_jet","#eta^{jets}", 1, true,true,true,true,true,     -3.0,   4.0,  false, false);
 example_stack(dir,"eta_1stMu","#eta^{#mu1}", 1, true,true,true,true,true,       -3.0,   3.5,  false, false);
 example_stack(dir,"eta_2ndMu","#eta^{#mu2}", 1, true,true,true,true,true,       -3.0,   3.5,  false, false);
 example_stack(dir,"multi_mu","N^{#mu}",1,true,true, true,true,true,       1,      5,  false, false);
 if(dir=="ControlP" || dir=="PreSel") example_stack(dir,"dR","#Delta R (#mu, jets)",1,true,true, true,true,true,       0,      6,  false, false);
 example_stack(dir,"final_multi_jet","N^{jets}",1,true,true,true,true,true,   0,      5,  false, false);
 example_stack(dir,"nvtx","N^{vertex}",1,true,true,true,true, true,           0.0,    70.0,  false, false);
 //example_stack(dir,"rhoAll","#rho",1,true,true,true,true, true,               0.0,    70.0,  false, false);
 if(dir=="ZTag"){
  example_stack(dir,"mjj_max","M_{#mu_{1}jj}^{max}",1,true,false,false,true, true,                 100,    3000.0, false, false);
  example_stack(dir,"mjj_min","M_{#mu_{2}jj}^{min}",1,true,false,false,true, true,                 0.0,    3000.0, false, false);
 }
}
