...
void makeCutFlowTableAny(ofstream &outFile, string sys, int Nbin, 
  ...
  outFile<<"\\multicolumn{5}{c}{ } \\\\"<<endl; 
  outFile<<"\\hline "<<endl;
  if(isMuonChannel){outFile<<"\\multicolumn{1}{| c|}{"+ sys +" } & \\multicolumn{1}{ c|}{ Muon trig } & \\multicolumn{1}{ c|}{ $N_{muon}=2$ } & \\multicolumn{1}{ c|}{ Muon SF } & \\multicolumn{1}{ c|}{ $Iso_{\\mu}^{rel} < 0.15$ } &\\multicolumn{1}{ c|}{ $M_{\\mu 1\\mu 2} > 60 GeV$ } & \\multicolumn{1}{ c|}{ $Pt_{\\mu 1\\mu 2} >100~GeV$ }&  \\multicolumn{1}{ c|}{ ZTag}  \\\\ "<<endl;
  }
  if(isEleChannel){outFile<<"\\multicolumn{1}{| c|}{"+ sys +" } & \\multicolumn{1}{ c|}{ Ele trig } & \\multicolumn{1}{ c|}{ $N_{ele}=2$ } & \\multicolumn{1}{ c|}{ Ele SF } & \\multicolumn{1}{ c|}{ $Iso_{e}^{rel} < 0.08$ } &\\multicolumn{1}{ c|}{ $M_{e1e2} > 60 GeV$ } & \\multicolumn{1}{ c|}{ $Pt_{e1e2} >100~GeV$ }&  \\multicolumn{1}{ c|}{ ZTag}  \\\\ "<<endl;
  }
  outFile<<"\\hline "<<endl;
  outFile<<"\\hline "<<endl;
  ...
  outFile<<printCutFlowHist("$M_{l*}=2000~GeV$", 	h_wh, 		Nbin, 1)<<" \\\\ "<<endl;
  outFile<<"\\hline "<<endl;
...
void getSignalCutFlowTable(ofstream &outFile, string sys, int Nbin, 
  ...
  outFile<<"\\hline "<<endl;  
  outFile<<printCutFlowHist("$M_{l*}=2000~GeV$",  h_wh120, 		Nbin, 1)<<" \\\\ "<<endl;
  outFile<<"\\hline "<<endl;  
...
void makeBaseTable(TString histSubDir="Iso/", TString histName="cutflow"){  
  ...
  //TFile *zjet  			= new TFile(inFile+"all_DY_amcatnlo.root");
  ...
  double Nbin = 8;
  makeCutFlowTableAny(outFile, "base", Nbin, h_wh_base,	h_ttbar_base, h_wjet_base, h_zjet_base, h_qcd_base, h_diboson_base, h_data);
  outFile<<"\\end{document}"<<endl;  
  outFile.close(); 
} 
...
void makeSummaryTableWithSys(TString histSubDir="Iso/", TString histName="cutflow"){  
  ...
  outFile<<"\\hline "<<endl; 
  outFile<<"\\multicolumn{1}{|c|}{Source} & \\multicolumn{1}{|c|}{N$_{\rm events}$ $\\pm$ MC stat $\\pm$ JES/JER} \\\\"<<endl;
  outFile<<"\\hline "<<endl; 
  outFile<<"\\hline "<<endl;
  int b = 7;
  outFile<<"$M_{l*} = 2000~GeV$"<<" & "<<h_wh_base->GetBinContent(b)<<" $\\pm$ "<<h_wh_base->GetBinError(b)<<" $\\pm$ "<< sqrt(pow(TMath::Max(fabs(h_wh_JESPlus->GetBinContent(b) - h_wh_base->GetBinContent(b)), fabs(h_wh_base->GetBinContent(b) - h_wh_JESMinus->GetBinContent(b))), 2) + pow(TMath::Max(fabs(h_wh_JERPlus->GetBinContent(b) - h_wh_base->GetBinContent(b)), fabs(h_wh_base->GetBinContent(b) - h_wh_JERMinus->GetBinContent(b))), 2)) <<" \\\\ "<<endl; 
  outFile<<"\\hline "<<endl;  
  outFile<<"SM $t\\bar{t}$"<<" & "<<h_ttbar_base->GetBinContent(b)<<" $\\pm$ "<<h_ttbar_base->GetBinError(b)<<" $\\pm$ "<< sqrt(pow(TMath::Max(fabs(h_ttbar_JESPlus->GetBinContent(b) - h_ttbar_base->GetBinContent(b)), fabs(h_ttbar_base->GetBinContent(b) - h_ttbar_JESMinus->GetBinContent(b))), 2) + pow(TMath::Max(fabs(h_ttbar_JERPlus->GetBinContent(b) - h_ttbar_base->GetBinContent(b)), fabs(h_ttbar_base->GetBinContent(b) - h_ttbar_JERMinus->GetBinContent(b))), 2))<< "\\\\ "<<endl;
  outFile<<"\\hline "<<endl;

  outFile<<"W+Jets"<<" & "<<h_wjet_base->GetBinContent(b)<<" $\\pm$ "<<h_wjet_base->GetBinError(b)<<" $\\pm$ "<< sqrt(pow(TMath::Max(fabs(h_wjet_JESPlus->GetBinContent(b) - h_wjet_base->GetBinContent(b)), fabs(h_wjet_base->GetBinContent(b) - h_wjet_JESMinus->GetBinContent(b))), 2) + pow(TMath::Max(fabs(h_wjet_JERPlus->GetBinContent(b) - h_wjet_base->GetBinContent(b)), fabs(h_wjet_base->GetBinContent(b) - h_wjet_JERMinus->GetBinContent(b))), 2))<< " \\\\ "<<endl;  
  outFile<<"\\hline "<<endl;  
  
  outFile<<"Z+Jets"<<" & "<<h_zjet_base->GetBinContent(b)<<" $\\pm$ "<<h_zjet_base->GetBinError(b)<<" $\\pm$ "<< sqrt(pow(TMath::Max(fabs(h_zjet_JESPlus->GetBinContent(b) - h_zjet_base->GetBinContent(b)), fabs(h_zjet_base->GetBinContent(b) - h_zjet_JESMinus->GetBinContent(b))), 2) + pow(TMath::Max(fabs(h_zjet_JERPlus->GetBinContent(b) - h_zjet_base->GetBinContent(b)), fabs(h_zjet_base->GetBinContent(b) - h_zjet_JERMinus->GetBinContent(b))), 2)) <<" \\\\ "<<endl;
  outFile<<"\\hline "<<endl;

  outFile<<"QCD"<<" & "<<h_qcd_base->GetBinContent(b)<<" $\\pm$ "<<h_qcd_base->GetBinError(b)<<" $\\pm$ "<< sqrt(pow(TMath::Max(fabs(h_qcd_JESPlus->GetBinContent(b) - h_qcd_base->GetBinContent(b)), fabs(h_qcd_base->GetBinContent(b) - h_qcd_JESMinus->GetBinContent(b))), 2) + pow(TMath::Max(fabs(h_qcd_JERPlus->GetBinContent(b) - h_qcd_base->GetBinContent(b)), fabs(h_qcd_base->GetBinContent(b) - h_qcd_JERMinus->GetBinContent(b))), 2)) <<" \\\\ "<<endl;
  outFile<<"\\hline "<<endl;
  ...
  outFile<<"\\hline "<<endl;
  outFile<<"\\hline "<<endl;
  outFile<<"Total Bkg"<<" & "<<h_TotalBkg->GetBinContent(b)<<" $\\pm$ "<<h_TotalBkg->GetBinError(b)<<" $\\pm$ "<<" -- "<<" \\\\ "<<endl;
  outFile<<"\\hline "<<endl;   
  outFile<<"\\hline "<<endl;  
  ...
  makeCutFlowTableAny(outFile, "base", Nbin, 		h_wh_base,  	h_ttbar_base, h_wjet_base, h_zjet_base, h_qcd_base,  h_diboson_base, h_data);
  makeCutFlowTableAny(outFile, "JESPlus", Nbin, 	h_wh_JESPlus, 	h_ttbar_JESPlus, h_wjet_JESPlus, h_zjet_JESPlus, h_qcd_JESPlus, h_diboson_JESPlus, h_data);
  makeCutFlowTableAny(outFile, "JESMinus", Nbin, 	h_wh_JESMinus, 	h_ttbar_JESMinus, h_wjet_JESMinus, h_zjet_JESMinus, h_qcd_JESMinus, h_diboson_JESMinus, h_data);
  makeCutFlowTableAny(outFile, "JERPlus", Nbin, 	h_wh_JERPlus, 	h_ttbar_JERPlus, h_wjet_JERPlus, h_zjet_JERPlus, h_qcd_JERPlus, h_diboson_JERPlus, h_data);
  makeCutFlowTableAny(outFile, "JERMinus", Nbin, 	h_wh_JERMinus, 	h_ttbar_JERMinus,h_wjet_JERMinus, h_zjet_JERMinus, h_qcd_JERMinus, h_diboson_JERMinus, h_data);
  outFile<<"\\end{document}"<<endl;  
  outFile.close(); 
} 
