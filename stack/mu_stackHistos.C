...
void example_stack(TString histname, TString xaxis_title, int bin, bool log=false, bool drawdata=true, bool ratio=false, bool drawsignal=false, bool axisrange=false, double xmin=0, double xmax=10, TString dir="BTag", bool label=false)
{
  ...
  TString inFile("$PWD/stack_20170919_SR_Mu_highMET/");
  ...
  TFile* fDa = TFile::Open(inFile+"all_muData.root");
  ...
  stackHisto(inFile, "all_TTJetsP.root","TTJets", histname, 433, 1.0134/0.9865, axisrange, xmin, xmax, MuptStack, hMC, hSig,leg);
  ...
  if(label){
    ...
    TString steps[15] = {"no cut","muon trig","= 1 muon","0 electron","muon SF","RelIso", "#geq 4 jets","#slash{E}_{T} #geq 20GeV", "#geq 2 b-jets","BTag SF","MT >0 GeV","Top Pt Rwt","KinFit noCut","KinFit cut",""};
  ...
  }else MuptStack->Draw("HIST"); // no histsame for dijet calculation
  ...
  bool savePDF = false;
  if(savePDF){
    TString outFile("/home/ravindra/lxplus/StackHisto/MuPlots/");
    outFile += histname;
    outFile += "_mu"+dir+".pdf";
    c1->SaveAs(outFile);
    c1->Close();
...
void example_stack_all(TString dir = ""){
  example_stack("cutflow", "cutflow", 1, true, true, true, false, true, 0, 15, dir, true);
  example_stack("pre_RelIso_mu", "RelIso of muons", 1, true, true, true, false, true, 0, 1, dir, false);
}

void example_stack_btagKF(TString dir="BTag"){//dir=KinFit, BTag
 //example_stack(dir+"/pt_mu","Pt^{#mu}[GeV]", 1, true,true,true,false, true,   0.0,    500.0, dir);
 //example_stack(dir+"/eta_mu","#eta^{#mu}", 1, true,true,true,false,true,       -2.5,   3.5,  dir);
 //example_stack(dir+"/pt_jet","Pt^{jets}[GeV]", 1, true,true,true,false, true,  0.0,    500,  dir);
 //example_stack(dir+"/eta_jet","#eta^{jets}", 1, true,true,true,false,true,     -2.5,   3.5,  dir);
 ...
 //example_stack(dir+"/mjj","m^{jj}",1,true,true,false,true, true,                0.0,    400.0, dir);
 example_stack(dir+"/mjj","m^{jj}",1,true,false,false,false, true,                0.0,    400.0, dir);
