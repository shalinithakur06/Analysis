...
TFile *fWH6000      = TFile::Open(inFileDir+"/all_signal6000.root");
...
TH1F* readWriteHisto(TFile *inFile, TString histPath, TString inHistName, double sf, TFile *outFile, TString outHistName, bool isWrite = false){
  ...
  if(!(inFile->Get(histPath+inHistName))){
    hist = (TH1F*)(fData->Get(histPath+inHistName))->Clone(outHistName);
    hist->Add(hist, -1);
...
//----------------------------------------//
void makeOneDataCard(TFile *fWH, int mass=2000, TString label="WH2000", TString label2="HH2000", TString histname = "mjj"){
...
void makeExLepDataCard_13TeV(){
  ...
  makeOneDataCard(fWH6000,  6000,  "WH6000",  "HH6000",  histname);
  fout->Close();
}
